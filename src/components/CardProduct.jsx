...
const CardProduct = ({key, name, type, image, price}) => {
    return (
        <section key={key} className="flex flex-col gap-2 bg-white rounded-3xl w-full p-3 shadow-md sm:w-52">
            <div className="relative">
                ...
                />
                <span className="absolute top-3 right-3 bg-primary-500 py-1 px-4 rounded-md text-white sm:text-xs" >{type}</span>
            </div>
            <section className="flex flex-col items-center justify-center gap-2 relative pb-10">
                ...
                <p className="bg-primary-500 py-1 px-4 rounded-md text-white text-lg sm:text-xs font-semibold">s/.{price}</p>
                <button
                type="button"
